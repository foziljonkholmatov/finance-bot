"""handlers/user/expense.py — Harajat qo'shish"""
...
from utils.states import ExpenseForm

router = Router()

MENU = {
    "💸 Harajat qo'shish","💸 Harajat qo'shish","📊 Bugungi hisobot",
...
async def income_start(message: Message, state: FSMContext) -> None:
    ...
    if not u or not u.get("is_registered"):
        ...
        return
    await state.clear()
    await state.set_state(ExpenseForm.currency)
    await message.answer("💸 <b>Harajat qo'shish</b>\n\nValyutani tanlang:",
                         parse_mode="HTML", reply_markup=currency_keyboard())


@router.callback_query(ExpenseForm.currency, F.data.startswith("currency:"))
async def income_currency(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    hints = {"UZS":"150000","USD":"50.5","EUR":"45.0"}
...
async def income_amount(message: Message, state: FSMContext) -> None:
    ...
    cats = await get_categories("expense")
...
@router.callback_query(ExpenseForm.category, F.data.startswith("cat:expense:"))
